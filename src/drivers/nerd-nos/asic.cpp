...
uint8_t Asic::sendWork(uint32_t job_id, bm_job_t *next_bm_job)
{
    BM1368_job job;
...
bool Asic::receiveWork(uint16_t timeout, asic_result_t *result)
{
    ...
    if (received != 11 || rcv_buf[0] != 0xAA || rcv_buf[1] != 0x55)
    {
...
bool Asic::processWork(uint16_t timeout, task_result *result)
{
    asic_result_t asic_result;
    if (!receiveWork(timeout,&asic_result)) {
        return false;
    }
